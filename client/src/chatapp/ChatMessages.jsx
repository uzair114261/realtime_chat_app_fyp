          ...
          <>
            <div className={`${receiverInfo ? "md:w-[40%] max-md:hidden" : "md:w-[70%]"} max-h-[100vh] relative  h-[100vh] overflow-auto ${!chatOpen && "max-md:hidden"}`}>
              <div className="flex justify-between items-center dark:bg-slate-900 bg-[#f0f2f5] h-[60px] px-1 md:px-3">
                <div className="flex">
                  <button className='mr-1 md:hidden' onClick={() => setChatOpen(false)}><ArrowLeft className='text-black dark:text-white' /></button>
                  <div className="receiver-info h-full flex items-center cursor-pointer" onClick={() => setReceiverInfo(true)}>
                    <div className='flex'>
                      <img className='avatar' src={`${process.env.REACT_APP_BACKEND_URL}${selectedUser.profileImage}`} alt="Receiver Profile" />
                    </div>
                    <div className=" ml-3">
                      <h5 className='text-md leading-4 mb-1 dark:text-white'>{selectedUser.firstName} {selectedUser.lastName}</h5>
                      <p className='text-xs md:text-sm leading-4 text-gray-500 dark:text-white'>last seen yesterday at 455 PM</p>
                    </div>
                  </div>
                </div>
                <div className='flex gap-5 items-center'>
                  <button className='' id='audioCall'><TelephoneFill className='text-[#54656f] dark:text-white' size={20} /></button>
                  <button className='' id='videoCall'><CameraVideoFill className='text-[#54656f] dark:text-white' size={20} /></button>
                  <button onClick={() => setShowDropdown(true)} type="button" className=" " id="menu-button" aria-expanded="true" aria-haspopup="true">
                    <ThreeDotsVertical size={20} className='text-[#54656f] dark:text-white' />
                  </button>
                  <div className="relative ">
                    <div ref={dropdownRef} className="relative inline-block text-left">
                      {
                        showDropdown && (
                          <div className="absolute right-[20px] z-10 mt-2 w-56 origin-top-right rounded-md bg-white dark:bg-slate-800 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
              ...
              <div className='h-[calc(100vh-60px)] max-h-[calc(100vh-60px)] overflow-y-auto overflow-x-hidden bg-[#AED8C7] dark:bg-slate-800 flex flex-col-reverse'>
                <div className="flex flex-col mb-20">
                  {filteredMessages.map((message, index) => (
                    <div key={index} className={` flex justify-between items-end py-1 px-2 text-sm rounded mt-1 mx-2 md:mx-4 relative ${message.sender === userData.id ? "self-end bg-[#D9FDD3] dark:bg-slate-700" : 'self-start bg-[#ffff] dark:bg-slate-600'}`}>
                      {message.image ? (
                        <div className={`max-w-[400px] h-[400px] bg-[${message.sender === userData.id ? '#D9FDD3' : '#fff'}] dark:bg-[${message.sender === userData.id ? 'slate-600' : 'slate-600'}]`}>
                          ...
                          <div className="flex justify-between items-center h-[20px]">
                            ...
                            <div className='max-w-[70px] text-gray-500 text-[10px] flex items-end'>{message.time}</div>
                          </div>
                        </div>
                      ) : (
                        <div className='max-w-[250px] min-w-[220px] flex'>
                          ...
                          <div className='max-w-[70px] min-w-[70px] text-gray-500 text-xs flex justify-end items-end dark:text-white'>{messageTime(message.timestamp)}</div>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className='absolute bottom-0 w-full bg-[#f0f2f5] dark:bg-slate-950 shadow-md px-4 h-[65px] flex items-center'>
                    ...
                    <div className='flex h-[50px] items-center justify-center'>
                      <div className="relative">
                        <div ref={dropdownRef} className="relative inline-block">
                          ...
                          {
                            showInputBox && (
                              <div className="absolute right-[-160px] top-[-130px] z-10 mt-2 w-[170px] origin-top-right rounded-md bg-white dark:bg-slate-800 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                <div className="py-1" role="none">
                                  <li className="hover:bg-gray-100 dark:text-white dark:hover:text-black ease-linear duration-150 text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                  ...
                  <input type="text" autoFocus value={sendTextMessage} onKeyPress={handlePressEnter} onChange={(event) => setSendTextMessage(event.target.value)} className='grow px-2 py-2 rounded outline-none dark:bg-slate-800 dark:text-white' placeholder='Type a message' />
          ...
          <>
            <div className='hidden bg-[#f0f2f5] dark:bg-slate-800 md:w-[70%] sm:flex items-center justify-center gap-5 flex-col'>
              <div className='text-center w-[70%]'>
                <h2 className="text-3xl text-gray-600 dark:text-white">Realtime Chat Application</h2>
                <p className='text-gray-600 text-sm dark:text-white'>Make audio & video calls and do messages to your friends and family and share multimedia files for faster and reliable experience</p>
              </div>
              <div className='text-center flex items-center gap-5'>
                <LockFill className='lock-icon dark:text-white' />  <p className='text-gray-600 text-sm dark:text-white'>Your messages and calls are end-to-end encrypted.</p>
              </div>
            </div>
          </>
        )
